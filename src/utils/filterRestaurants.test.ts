...
describe('filterRestaurants', () => {
  ...
  context('with "한식" category', () => {
    it('return all restaurants', () => {
      ...
      expect(filteredRestaurants[0].category).toBe('한식');
    });
  });
  ...
  context('with "중식" category', () => {
    it('return all restaurants', () => {
      ...
      expect(filteredRestaurants.length).toBe(1);
      expect(filteredRestaurants[0].category).toBe('중식');
    });
  });
  ...
  context('with "일식" category', () => {
    it('return all restaurants', () => {
      ...
      expect(filteredRestaurants.length).toBe(1);
      expect(filteredRestaurants[0].category).toBe('일식');
    });
  });

  context('with filterText', () => {
    it('return filtered restaurants', () => {
      const filteredRestaurants = filterRestaurants(
        restaurants,
        { filterText: '메', filterCategory: '전체' },
      );
